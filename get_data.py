...
def get_players_data(players):
    ...
    try:
        ...
        # 3. The response is added to the dataframe
            ...
            #df_temp.drop('index', axis = 1)
            df = df.append(df_temp, True)
            df = df.drop(df.columns[[0]], axis=1)
            #df.columns[0] = df.columns[0].tolist()
            print(type(df["id"]))
...
def main():
    ...
    #pls2 = ["e_0shams0", "Andrej1909", "Lassandra", "Ayowole", "thibault","cyanfish","agadmator","Oliver_Penrose","bmartin","ismodes","penguingim1","Zhigalko_Sergei","mbellm"]
